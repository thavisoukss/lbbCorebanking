...
//            );
    ...
    public <R> R getStatementDetailIn(String url, HttpHeaders headers, Class<R> responseType) {
        logger.info("Calling External API  Statement (GET): URL={}", url);
...
//            );
    ...
    public <R> R getStatementDetailOut(String url, HttpHeaders headers, Class<R> responseType) {
        logger.info("Calling External API  Statement (GET): URL={}", url);
    ...
    public <T, R> R internalTransfer(String url, T requestBody, HttpHeaders headers ,Class<R> responseType ) {
        logger.info("Calling External API: URL={}", url);

        HttpEntity<T> entity = new HttpEntity<>(requestBody, headers);

        try {
            logger.info("start call api internal transfer  request : {}", requestBody);

            R response = restTemplate.postForObject(url, entity, responseType);

            logger.info("end call api internal transfer   response : {}", response);
            return response;
    ...
    public <T, R> R topup(String url, T requestBody, HttpHeaders headers ,Class<R> responseType ) {
        ...
        try {
            logger.info("start call api top-up request : {}", requestBody);

            R response = restTemplate.postForObject(url, entity, responseType);

            logger.info("end call api api top-up   response : {}", response);
            return response;
